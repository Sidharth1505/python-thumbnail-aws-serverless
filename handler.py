...
def upload_to_s3(bucket,thumbnail_key,image,img_size):
    ...
    s3_save_thumbnail_to_dynamo(url_path=url,img_size=img_size)
...
def s3_save_thumbnail_to_dynamo(url_path,img_size):
    ...
    response = table.put_item(
        Item={
            ...
            'createdAt':str(datetime.time()),
            'updatedAt':str(datetime.time())
        }
    )
...
def s3_delete_item(event,context):
    ...
    #if the response the successful that is deletion is successful
